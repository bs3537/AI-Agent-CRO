...
from .heuristic import red_team_heuristically
    ...
    *,
    ...
    if provider is None:
        def runner(c: RedTeamCandidate, cat: Catalog) -> tuple[RedTeamResult, str]:
            return red_team_heuristically(c, cat), HEURISTIC_MODEL

        runner_label = HEURISTIC_MODEL
    else:
        def runner(c: RedTeamCandidate, cat: Catalog) -> tuple[RedTeamResult, str]:
            return red_team_with_llm(c, cat, provider=provider)
    ...
    results = map_concurrent(_run_candidate, candidates, workers=workers)
